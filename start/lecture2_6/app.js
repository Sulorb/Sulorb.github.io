...
class App {
    ...
    loadGLTF() {
        ...
        loader.load('office-chair.glb', (gltf) => {
            ...
            console.log(`min: ${vector3ToString(bbox.min)} max: ${vector3ToString(bbox.max)}`);
            


            self.scene.add(gltf.scene);
            self.loadingBar.visible = false;
            self.renderer.setAnimationLoop(this.render.bind(this));
        },
            (xhr) => {
                self.loadingBar.progress = xhr.loaded / xhr.total;
            },
            (err) => {
                console.error('An error occurred loading the GLTF', err);
            }
        );
    }
